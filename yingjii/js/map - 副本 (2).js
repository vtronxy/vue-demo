    ...
    window.pmap = {
        ...
        loadStaNumChange: function() {
            pmap.Timer.station = setInterval(function() {
                ...
                var a = (Math.random() > .5 ? 1 : -1) * parseInt(Math.random() * 50 + 50);
        ...
        loadStatDanger: function() {
            ...
            pmap.Timer.zdcs = setTimeout(function() {
                ...
                pmap.playStarTrack(d, map_pos.zdcs, function() {
                    c.dataStat.Update(1);
                    ggaq.load_data();
                    pmap.loadStatDanger();
        ...
        loadCaseTimer: function() {
            ...
            pmap.Timer.jtsg = setInterval(function() {
                pmap.loadRandCase("0", function(b) {
                    pmap.playStarTrack(b, map_pos.jtsg, function() {
                        $("#jt_sgs").text(parseInt($("#jt_sgs").text()) + 1);
                    });
                });
            }, (1 * a + 10) * 1e3);
            pmap.Timer.zpwg = setInterval(function() {
                pmap.loadRandCase("2", function(b) {
                    pmap.playStarTrack(b, map_pos.zpwg, function() {
                        $("#jt_zpwg").text(parseInt($("#jt_zpwg").text()) + 1);
                    });
                });
            }, (5 * a - 20) * 1e3);
            pmap.Timer.jttel = setInterval(function() {
                pmap.loadRandCase("1", function(b) {
                    pmap.playStarTrack(b, map_pos.jttel, function() {
        ...
        loadChuTian25d: function() {
			var a = new maptalks.TileLayer("tile", {
                ...
                'urlTemplate' : function (x, y, z) {
                    var i = 0, domain = '';
                    for(; i < z; i++) {
                        domain = ((x & 1) + 2 * (y & 1)).toString();
                    }
                    return 'http://q' + domain + '.o.cn/resource/mappic/21/3/lv' + (6 - z) + '/' + x + ',' + y + '.jpg';
                }
            });
            a.on("layerload", function() {
                if (pmap.map25d == null) {
                    return;
                }
        ...
        load_sdqyxfw_layer: function(d, f) {
            ...
            if (pmap.layer_sdqyxfw != null && pmap.sdqyfw_yxarea != null) {
                ...
                if (pmap.sdqyfw_yxarea != null) {
                    pmap.sdqyfw_yxarea.on("mouseover", function(h) {
                        var g = h.target.getSymbol();
                        g.polygonOpacity = .5;
                        h.target.setSymbol(g);
                    });
                    pmap.sdqyfw_yxarea.on("mouseout", function(h) {
                        var g = h.target.getSymbol();
                        g.polygonOpacity = .3;
                        h.target.setSymbol(g);
                    });
